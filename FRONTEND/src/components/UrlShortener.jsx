...
const DashboardOverviewContent = ({user}) => (
  ...
  <p className="mt-2 text-sm md:text-base text-gray-500">Glad to see you back on your dashboard 🎉</p>
    ...
    <div className="mt-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      <div className="p-4 bg-green-50 rounded-xl border border-green-200 text-green-800 shadow-sm">
        <h3 className="text-lg font-medium mb-2">Total Clicks</h3>
        <p className="text-3xl font-bold">5,000</p>
      </div>
      <div className="p-4 bg-purple-50 rounded-xl border border-purple-200 text-purple-800 shadow-sm">
...
const MyUrlsContent = ({ openQrModal, showToast }) => {
  ...
  const handleDelete = async (id) => {
    showToast(`Attempting to delete URL with ID: ${id}`, 'info');
              ...
              <tr key={url._id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  <div className="flex items-center space-x-2">
                    ...
                    <button onClick={openQrModal} className="text-blue-600 hover:text-blue-900 p-1 rounded-full hover:bg-gray-100" title="View QR Code">
...
const CreateShortLinkContent = ({ openQrModal, showToast }) => {
    ...
    <div className="bg-white p-6 rounded-2xl shadow-md border border-gray-200">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="flex flex-col sm:flex-row items-center pt-4 space-y-4 sm:space-y-0 sm:space-x-4">
          <button type="button" onClick={openQrModal} className="flex items-center justify-center px-4 py-2 border border-gray-300 rounded-xl shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 w-full sm:w-auto">
...
const CustomAliasContent = ({ showToast }) => {
  const [longUrl, setLongUrl] = useState('');
  const [customAlias, setCustomAlias] = useState('');
  const [shortenedUrl, setShortenedUrl] = useState('');
  const API_BASE_URL = 'http://localhost:3000';
...
const FireLinksContent = ({ showToast }) => {
  const [longUrl, setLongUrl] = useState('');
  const [customAlias, setCustomAlias] = useState('');
...
const QrCodeGeneratorContent = ({ openQrModal }) => (
  <div className="bg-white p-6 rounded-2xl shadow-md border border-gray-200">
    ...
    <div className="space-y-4">
      ...
      <button onClick={openQrModal} className="flex items-center justify-center px-6 py-3 border border-transparent rounded-xl shadow-sm text-base font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
...
const TopNavbar = ({ currentPageTitle, toggleMobileSidebar,user }) => (
  <div className="flex items-center justify-between bg-white shadow-sm p-4 border-b border-gray-200">
    <div className="flex items-center">
      ...
      <div className="text-xl font-semibold text-gray-800">{currentPageTitle}</div>
    </div>
...
const QrCodeModal = ({ imageUrl = "https://via.placeholder.com/250", onClose }) => (
  <div className="fixed inset-0 bg-gray-600 bg-opacity-75 flex justify-center items-center p-4 z-50">
    <div className="bg-white rounded-2xl shadow-xl border border-gray-200 w-full max-w-sm relative">
      ...
      <div className="p-6 flex flex-col items-center">
        <img src={imageUrl} alt="QR Code" className="w-64 h-64 border border-gray-300 rounded-xl mb-6" />
        <button className="flex items-center justify-center px-4 py-2 border border-transparent rounded-xl shadow-sm text-base font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
...
const DashboardUI = () => {
  const [currentPage, setCurrentPage] = useState('CustomAlias');
    ...
    <div className="flex h-screen bg-gray-100 font-sans">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-x-hidden overflow-y-auto p-4 md:p-6">
          ...
          {currentPage === 'FireLinks' && <FireLinksContent showToast={showToast} />}
          {currentPage === 'LocationBasedLink' && <LocationBasedLinkContent />}
          {currentPage === 'QrCodeGenerator' && <QrCodeGeneratorContent openQrModal={openQrModal} />}
